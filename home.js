...
function showItems(products) {
  ...
  products.forEach(product => {
    ...
    a.href = `product.html?id=${product.id}`; // or any dynamic route you want
    ...
    div.innerHTML = `
      <img width="100%" height="auto" src="${product.image}" alt="${product.name}" class="item-img" />
      <h3>${product.name}</h3>
      <p>₹${product.price}</p>
    `;
    ...
    btn.onclick = (e) => {
      e.preventDefault(); // prevents <a> from triggering when clicking button
